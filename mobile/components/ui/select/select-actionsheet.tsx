...
const actionsheetContentStyle = tva({
  base: "items-center rounded-t-xl p-2 bg-background-0 web:pointer-events-auto web:select-none shadow-lg pb-safe ",
});

const actionsheetItemStyle = tva({
  base: "w-full flex-row  items-center p-3 rounded-xl data-[disabled=true]:opacity-40 data-[disabled=true]:web:pointer-events-auto data-[disabled=true]:web:cursor-not-allowed hover:bg-background-50 active:opacity-50 data-[focus=true]:bg-background-100 web:data-[focus-visible=true]:bg-background-100 data-[checked=true]:bg-primary-700 ",
});

const actionsheetItemTextStyle = tva({
  base: "text-typography-0 font-normal tracking-md text-left mx-2  ",
  variants: {
    isTruncated: {
      true: "",
    },
...
>(function ActionsheetItem({ children, className, ...props }, ref) {
  const checked =
    props.dataSet?.checked === true || props.dataSet?.checked === "true";
  return (
    <UIActionsheet.Item
...
const ActionsheetItemText = React.forwardRef<
  ...
  IActionsheetItemTextProps & { checked?: boolean | string }
  ...
  return (
    <UIActionsheet.ItemText
      className={actionsheetItemTextStyle({
        class: `${className ?? ""} ${checked ? "text-primary-0" : ""}`,
